...
class BasePredictor:
    ...
    def __init__(self, cfg=None, **kwargs):
        ...
        self.cfg = cfg or kwargs or {}
        
        # 解析超参数配置 (支持从 hyp.yaml 读取 conf, iou 等推理参数)
        self.hyp = {}
        hyp_path = self.cfg.get('hyp', None)
        
        if hyp_path and os.path.exists(str(hyp_path)):
            with open(str(hyp_path), "r", encoding="utf-8") as f:
                self.hyp = yaml.safe_load(f)

        self.conf_thres = self.hyp.get('conf', self.cfg.get('conf', 0.25))
        self.iou_thres = self.hyp.get('iou', self.cfg.get('iou', 0.45))
    ...
    def __call__(self, source, model=None, ckpt_path=None):
        ...
        save_dir = self.cfg.get('save_dir', 'runs/detect/predict')
        should_save = self.cfg.get('save', True) 
        
        if should_save:
            os.makedirs(save_dir, exist_ok=True)
            LOGGER.info(f" 推理结果将保存至: {os.path.abspath(save_dir)}")
        ...
        for img_path_or_arr in self.dataset:
            ...
            # 后处理 (这里会返回各任务自定义的 Results 实例)
            t3 = time.time()
            result = self.postprocess(preds, orig_img, prep_info)
            
            if should_save and result is not None:
                if isinstance(img_path_or_arr, str):
                    fname = os.path.basename(img_path_or_arr)
                else:
                    fname = f"result_{int(time.time()*1000)}.jpg"
                
                if hasattr(result, 'save'):
                    result.save(save_dir=save_dir, file_name=fname)
                    result.save_dir = os.path.abspath(save_dir)
            ...
            LOGGER.info(f"处理完成 [{os.path.basename(name) if isinstance(name, str) else 'arr'}] | 前向推理: {inf_time:.1f}ms | 后处理: {post_time:.1f}ms")
