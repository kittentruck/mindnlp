...
from ultralytics.modules import ConvNormAct, C3k2, C2PSA, Classify, YOLO11DetectHead, YOLO11Segment, YOLO11Pose, Concat, Identity, SPPF, Upsample
...
class YOLO11Base(nn.Cell):
    """
    YOLO11 通用基类，负责解析 YAML 配置、搭建网络拓扑结构以及权重的初始化。
    """
    
    def __init__(self, config):
        # nn.Cell 的初始化不需要传 config
        super().__init__() 
        self.config = config
        self.nc = config.nc
