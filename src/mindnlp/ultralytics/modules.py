...
class YOLO11DetectHead(nn.Cell):
    ...
    def construct(self, x):
        ...
        # 拼接所有尺度，得到形状: [B, 144, 19200] (以 640x640 输入为例)
        x_cat = ops.concat(y, 2)
        
        if self.training:
            return x_cat, res

        # 将预测值拆分为 框回归参数(box_dist) 和 类别概率(cls_logits)
        box_dist, cls_logits = ops.split(x_cat, (self.reg_max * 4, self.nc), axis=1)
        
        # DFL 积分回归 -> 得到形状 [B, 4, 19200]
        box_decoded = self.dfl(box_dist) 
        
        # 获取原始锚点和步长
        anchors, strides = make_anchors(x, self.stride, 0.5)
        
        anchors_aligned = anchors.transpose(1, 0).expand_dims(0)
        
        # 同样旋转并增加维度，使其成为横向的步长向量
        strides_aligned = strides.transpose(1, 0).expand_dims(0)
        
        # 解码坐标：使用显式对齐后的张量进行计算
        # dbox 形状将保持为 [B, 4, 19200]
        dbox = dist2bbox(box_decoded, anchors_aligned, strides=strides_aligned, xywh=True, axis=1)

        # 最终拼接：[B, 4+nc, 19200]
        # 注意对类别概率进行 sigmoid 处理
...
class YOLO11Pose(YOLO11DetectHead):
    ...
    def construct(self, x):
        ...
        box_flat = ops.concat([b.view(bs, self.reg_max * 4, -1) for b in box_outs], axis=2).astype(ms.float32)
        cls_flat = ops.concat([c.view(bs, self.nc, -1) for c in cls_outs], axis=2).astype(ms.float32)
        kpt_flat = ops.concat([k.view(bs, self.nkpt, -1) for k in kpt_outs], axis=2).astype(ms.float32)

        anchors, strides = make_anchors(x, self.stride, 0.5)
        anc_aligned = anchors.transpose(1, 0).expand_dims(0)
        strd_aligned = strides.transpose(1, 0).expand_dims(0)

        box_dist = self.dfl(box_flat)
        dbox = dist2bbox(box_dist, anc_aligned, strides=strd_aligned, xywh=True, axis=1)

        pred_kpt = self.kpts_decode(kpt_flat, anchors, strides)

        y = ops.concat((dbox, ops.sigmoid(cls_flat), pred_kpt), axis=1) 
...
class YOLO11Segment(YOLO11DetectHead):
    ...
    def construct(self, x):
        ...
        anchors, strides = make_anchors(out, self.stride, 0.5)
        ...
        box_decoded = self.dfl(box) # 得到 [B, 4, N]
        dbox = dist2bbox(box_decoded, anc_aligned, strides=strd_aligned, xywh=True, axis=1)
